...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 24 17:57:54 2018
...
#########################################
# 0 base for this experiment - lgbm trainined on all the features
#########################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '0', 10000, 
...
##############################################
# 1 adding sum for the categorical features 
##############################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '1', 10000, 
                                      "LGB trained for ALL the features", 'LGB')
...
##############################################
# 2 adding more installment features
##############################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '2', 10000, 
                                      "LGB trained for ALL the features and more installment features NLP_Y, DPD_Y, DBD_Y", 'LGB')

important_features.sort_values()


##############################################
# 3 adding more installment features
##############################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '3', 10000, 
                                      "LGB trained for ALL the features and more installment features DPD_400, DPD_800", 'LGB')
...
# 4 5 fold lightGBM
####################################################
feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_15.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()

####################################################
# 5 more features on installment payments
####################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '5', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature", 'LGB')

important_features.sort_values()

####################################################
# 6 remove duplicates from prev applications
####################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '6', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and remove prev duplicates", 'LGB')

important_features.sort_values()

####################################################
# 7 adding total prev application etc
####################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '7', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and total prev applications", 'LGB')
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '8', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features", 'LGB')

important_features.sort_values()

####################################################
# 9 extra CC and POS features
####################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '9', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS", 'LGB')

important_features.sort_values()


###############################################################
# 10 extra CC and POS features removing cc dpd features
###############################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '10', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS", 'LGB')
...
# 11 5 fold lightGBM
####################################################
feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_16.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()

###############################################################
# 12 extra CC and POS features removing cc dpd features andding install dpd slopes 
###############################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '12', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS", 'LGB')

important_features.sort_values()

###############################################################
# 13 extra CC and POS features removing cc dpd features andding install dpd slopes 
###############################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '13', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS more slope features", 'LGB')

important_features.sort_values()


important_features = list(important_features[important_features > 0].index)

############## remove extra features

clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '14', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS more slope features remove 0 imp columns", 'LGB')
...
# 14 5 fold lightGBM
####################################################
feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_17.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()


###############################################################
# 15 extra CC and POS features removing cc dpd features andding install dpd slopes mean ext ratings
###############################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '16', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS more slope features and mean ext rating", 'LGB')

important_features.sort_values()


###############################################################
# 17 some new features
###############################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_13', '17', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS more slope features and mean ext rating and new features", 'LGB')

important_features.sort_values()

############### mew submission

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_18.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()
