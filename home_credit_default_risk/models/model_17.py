...
def get_application_train_test(nan_as_category=False):
    ...
    #one_hot_df['NEW_SOURCES_PROD_1_2'] = one_hot_df['EXT_SOURCE_1'] * one_hot_df['EXT_SOURCE_2'] 
    #one_hot_df['NEW_SOURCES_PROD_2_3'] = one_hot_df['EXT_SOURCE_2'] * one_hot_df['EXT_SOURCE_3']
    #one_hot_df['NEW_SOURCES_PROD_1_3'] = one_hot_df['EXT_SOURCE_1'] * one_hot_df['EXT_SOURCE_3']
    one_hot_df['INCOME_PER_AREA'] = one_hot_df['AMT_INCOME_TOTAL'] * one_hot_df['REGION_POPULATION_RELATIVE']
    #one_hot_df['INCOME_PER_REGION_AREA'] = one_hot_df['AMT_INCOME_TOTAL'] * one_hot_df['REGION_RATING_CLIENT']
    #one_hot_df['INCOME_PER_REGION_W_AREA'] = one_hot_df['AMT_INCOME_TOTAL'] * one_hot_df['REGION_RATING_CLIENT_W_CITY']

    #one_hot_df['NEW_OBS_30_CNT_SOCIAL_CIRCLE'] = one_hot_df['OBS_30_CNT_SOCIAL_CIRCLE']/one_hot_df['DAYS_BIRTH']
    #one_hot_df['NEW_DEF_30_CNT_SOCIAL_CIRCLE'] = one_hot_df['DEF_30_CNT_SOCIAL_CIRCLE']/one_hot_df['DAYS_BIRTH']
    #one_hot_df['NEW_OBS_60_CNT_SOCIAL_CIRCLE'] = one_hot_df['OBS_60_CNT_SOCIAL_CIRCLE']/one_hot_df['DAYS_BIRTH']
    #one_hot_df['NEW_DEF_60_CNT_SOCIAL_CIRCLE'] = one_hot_df['DEF_60_CNT_SOCIAL_CIRCLE']/one_hot_df['DAYS_BIRTH']
...
def get_bureau_features(nan_as_category = False):
    ...
    agg_dict= {
               ...
               'AMT_CREDIT_DEBT_RATE': ['min', 'max', 'mean', 'std']
               }
    ...
    # Bureau: Closed credits - using only numerical aggregations
    crt_800 = bureau[bureau['DAYS_CREDIT'] >= -800]
    crt_800 = crt_800.groupby('SK_ID_CURR').agg(agg_dict)
    crt_800.columns = pd.Index(['800_' + e[0] + "_" + e[1].upper() for e in crt_800.columns.tolist()])
    crt_800 = crt_800.reset_index()
    
    bureau_features = pd.merge(bureau_features, crt_800, how='left', on='SK_ID_CURR')
    
    del bureau, closed, closed_agg, crt_800
    gc.collect()
...
def get_bureau_balance_features(nan_as_category=False):
    ...
    bureau_balance = bureau_balance[bureau_balance.MONTHS_BALANCE > -24]
...
def get_installments_payments_features():
    ...
    ins = ins[ins.DAYS_INSTALMENT >= -1100]
...
###################################################
# add more better feaures to improve CV 
###################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '0', 10000, 
...
#################################################
important_columns.remove('NEW_OBS_30_CNT_SOCIAL_CIRCLE')
important_columns.remove('NEW_OBS_60_CNT_SOCIAL_CIRCLE')
important_columns.remove('NEW_DEF_30_CNT_SOCIAL_CIRCLE')
important_columns.remove('NEW_DEF_60_CNT_SOCIAL_CIRCLE')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '1', 10000, 
                                      "LGB trained for ALL the features + income per area", 'LGB')

##################################################
important_columns.remove('NEW_SOURCES_PROD_1_2')
important_columns.remove('NEW_SOURCES_PROD_2_3')
important_columns.remove('NEW_SOURCES_PROD_1_3')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '2', 10000, 
                                      "LGB trained for ALL the features + income per area", 'LGB')

#################################################
important_columns.append('NEW_OBS_30_CNT_SOCIAL_CIRCLE')
important_columns.append('NEW_OBS_60_CNT_SOCIAL_CIRCLE')
important_columns.append('NEW_DEF_30_CNT_SOCIAL_CIRCLE')
important_columns.append('NEW_DEF_60_CNT_SOCIAL_CIRCLE')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '3', 10000, 
                                      "LGB trained for ALL the features + income per area + CNT social", 'LGB')

#################################################
important_columns.remove('INCOME_PER_REGION_AREA')
important_columns.remove('INCOME_PER_REGION_W_AREA')
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '5', 10000, 
                                      "LGB trained for ALL the features - buro recent features", 'LGB')
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '6', 10000, 
                                      "LGB trained for ALL the features -BB > 24", 'LGB')
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '7', 10000, 
                                      "LGB trained for ALL the features -BB > 24 POS > 24", 'LGB')
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '8', 10000, 
                                      "LGB trained for ALL the features -BB > 24 POS > 24 cc> 24", 'LGB')

important_features.sort_values() 


################################################
# INS payment recent features
################################################
whole_df = get_application_train_test()
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '9', 10000, 
                                      "LGB trained for ALL the features -BB > 24 POS > 24 ins> 36", 'LGB')

important_features.sort_values() 


################################################
# INS with CC payment recent features
################################################
whole_df = get_application_train_test()
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '10', 10000, 
                                      "LGB trained for ALL the features -BB > 24 POS > 24 ins> 36 cc> 24", 'LGB')
...
################################################
# INS without CC payment recent features
################################################
whole_df = get_application_train_test()
...
#############################################
# Run 5 fold stratified and non-stratified
# with differnt seeds and take average
###########################################
with open('whole_df', 'rb') as f:
    whole_df = pickle.load(f)

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_28.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_29.csv', stratified= True, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()
...
feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_30.csv', stratified= True, debug=False)

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_31.csv', stratified= True, debug=False)

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_32.csv', stratified= True, debug=False)

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_33.csv', stratified= True, debug=False)

######################################
submission_29 = pd.read_csv('submission_29.csv')
submission_30 = pd.read_csv('submission_30.csv')
submission_31 = pd.read_csv('submission_31.csv')
submission_32 = pd.read_csv('submission_32.csv')
submission_33 = pd.read_csv('submission_33.csv')

final = pd.merge(submission_29, submission_30, on='SK_ID_CURR', suffixes=('_x', '_y'))
final = pd.merge(final, submission_31, on='SK_ID_CURR', suffixes=('_x', '_y'))
final = pd.merge(final, submission_32, on='SK_ID_CURR', suffixes=('__x', '__y'))
final = pd.merge(final, submission_33, on='SK_ID_CURR', suffixes=('_x', '_y'))

final['TARGET_new'] = (final['TARGET']+final['TARGET_x']+final['TARGET_y']+final['TARGET__x']+final['TARGET__y'])/5
     
submission = pd.DataFrame({'SK_ID_CURR': final['SK_ID_CURR'], 'TARGET': final['TARGET_new']})
submission.to_csv('submission_34.csv', index=False)     
     
#######################################
# buro debit creidt ratio 
#######################################
whole_df = get_application_train_test()
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_17', '11', 10000, 
                                      "LGB trained for ALL the features - added buro debit credit ratio", 'LGB')
