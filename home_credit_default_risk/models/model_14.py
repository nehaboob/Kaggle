...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '1', 10000, 
...
important_features.sort_values().to_csv('imp_features_2.csv')
            
#############################################################
# 1 base for this file
#############################################################
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '1', 10000, 
                                      "LGB trained for ALL the features and prev installmante payment rate feature and cc features and POS more slope features and mean ext rating and new features and more features", 'LGB')
...
###############################
# get eyeball predictions
###############################
    
train_all_skid, dev_all_skid, dev_eyeball_skid = get_train_dev_sets()

X_dev_eye = application_train[application_train.SK_ID_CURR.isin(dev_eyeball_skid)]

y_dev_eye = y_true.loc[y_true.SK_ID_CURR.isin(dev_eyeball_skid), 'TARGET']

y_pred = clf.predict(X_dev_eye[important_columns])
y_pred_proba = clf.predict_proba(X_dev_eye[important_columns])[:,1]

X_dev_eye['TARGET'] = y_dev_eye
X_dev_eye['Y_PRED'] = y_pred
X_dev_eye['Y_PRED_PROBA'] = y_pred_proba
     
columns = list(important_features[(important_features > 0.2)].index)
columns.append('TARGET')    
columns.append('Y_PRED')         
columns.append('Y_PRED_PROBA')         
     
X_dev_eye[columns].to_csv('DEV_set_eyeball.csv', index=False)

# ext ratings are high for people

#############################################################
# 2 without ext ratings
#############################################################
...
important_columns = list(application_train.columns)
important_columns.remove('SK_ID_CURR')
important_columns.remove('EXT_SOURCE_1')
important_columns.remove('EXT_SOURCE_2')
important_columns.remove('EXT_SOURCE_3')
important_columns.remove('NEW_EXT_SOURCES_MEAN')
important_columns.remove('NEW_SOURCES_PROD')
important_columns.remove('NEW_SCORES_STD')
important_columns.remove('index')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '3', 10000, 
                                      "LGB trained for ALL the features and without EXT ", 'LGB')
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '4',500, 
                                      "LGB trained for ALL the features and without EXT ", 'GBC')

important_features.sort_values()

#########################################################
# LGB with balanced class weights
###########################################################
whole_df = get_application_train_test()
...
clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '5', 10000, 
                                      "LGB trained for ALL the features and balanced class weights ", 'LGB')

important_features.sort_values()

##########################################################
# check what happens when we change probability values
# Not working :()
##########################################################
       
train_all_skid, dev_all_skid, dev_eyeball_skid = get_train_dev_sets()

X_dev = application_train[application_train.SK_ID_CURR.isin(dev_all_skid)]

y_dev = y_true.loc[y_true.SK_ID_CURR.isin(dev_all_skid), 'TARGET']

y_dev_predict = clf.predict(X_dev[important_columns].fillna((0)))
y_dev_predict_proba = clf.predict_proba(X_dev[important_columns].fillna(0))

get_scores(y_dev, y_dev_predict, y_dev_predict_proba[:, 1], 'dev')

y_dev_predict_proba_new = y_dev_predict_proba[:, 1]
y_dev_predict_proba_new[(y_dev_predict_proba_new > 0.5)] = 0.99
 
y_dev_predict_new = (y_dev_predict_proba_new > 0.5)*1
                        
get_scores(y_dev, y_dev_predict_new, y_dev_predict_proba_new, 'dev')          
             

####################################################################
# 5 experiment with more class weights
####################################################################
clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '5', 10000, 
                                      "LGB trained for ALL the features and scale positive weight 2", 'LGB')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '6', 10000, 
                                      "LGB trained for ALL the features and scale positive weight 5", 'LGB')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '7', 10000, 
                                      "LGB trained for ALL the features and scale positive weight 10", 'LGB')

clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '8', 10000, 
                                      "LGB trained for ALL the features and scale positive weight 1.3", 'LGB')

##################################################################
#
##################################################################

feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_19.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()

##################
clf, important_features = train_model(application_train, y_true, important_columns, 'model_14', '8', 10000, 
                                      "LGB trained for ALL the features and no class weight", 'LGB')

########################
feat_importance = kfold_lightgbm(whole_df, num_folds= 5, submission_file_name='submission_20.csv', stratified= False, debug=False)
feat_importance.groupby('feature')['importance'].sum().sort_values()

##########################
# take average of submission_18.csv, submission_19.csv, submission_20.csv
###########################

submission_18 = pd.read_csv('submission_18.csv')
submission_19 = pd.read_csv('submission_19.csv')
submission_20 = pd.read_csv('submission_20.csv')


final = pd.merge(submission_18, submission_19, on='SK_ID_CURR')
final = pd.merge(final, submission_20, on='SK_ID_CURR')

final['total'] = (final['TARGET_x'] + final['TARGET_y'] +final['TARGET'])/3


(final.total > 0.5).sum()
(submission_18.TARGET > 0.5).sum()
(submission_19.TARGET > 0.5).sum()
(submission_20.TARGET > 0.5).sum()

submission = pd.DataFrame({'SK_ID_CURR': final['SK_ID_CURR'], 'TARGET': final['total']})
submission.to_csv('submission_21.csv', index=False)

(submission.TARGET > 0.5).sum()
