...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 14 11:21:57 2018
...
submission_file = 'submission_15_indi_th.csv'
...
preds_valid1 = np.load('preds_valid_rev_sgd_lh_aug_lowlr_1.npy')    
preds_valid2 = np.load('preds_valid_rev_sgd_lh_aug_lowlr_2.npy')    
...
y_valid1 = np.load('y_valid_rev_sgd_lh_aug_lowlr_1.npy')    
y_valid2 = np.load('y_valid_rev_sgd_lh_aug_lowlr_2.npy')    
y_valid3 = np.load('y_valid_rev_sgd_lh_aug_3.npy')     
y_valid4 = np.load('y_valid_rev_sgd_lh_aug_4.npy')

preds_test0 = np.load('preds_test_rev_sgd_lh_aug_0.npy')    
preds_test1 = np.load('preds_test_rev_sgd_lh_aug_lowlr_1.npy')    
preds_test2 = np.load('preds_test_rev_sgd_lh_aug_lowlr_2.npy')    
preds_test3 = np.load('preds_test_rev_sgd_lh_aug_3.npy')    
preds_test4 = np.load('preds_test_rev_sgd_lh_aug_4.npy') 


final_prediction = preds_valid4
final_truth = y_valid4
sumbit_prediction = preds_test4

                   
final_prediction = [downsample(img) for img in final_prediction]
final_truth = [downsample(img) for img in final_truth]   
sumbit_prediction = [downsample(img) for img in sumbit_prediction]
    
final_prediction = np.array(final_prediction)
final_truth =np.array(final_truth)
sumbit_prediction = np.array(sumbit_prediction)
## Scoring for last model, choose threshold by validation data 
thresholds_ori = np.linspace(0.3, 0.9, 40)
...
TH - 0.6359, 1.1611, 0.9985, 1.5373, 0.7750
IOU - 0.8320 + 0.8264 + 0.8258 + 0.8343 + 0.8245
"""

preds_test0 = np.multiply((preds_test0 > 0.6359), 1)   
preds_test1 = np.multiply((preds_test1 > 1.1611), 1)   
preds_test2 = np.multiply((preds_test2 > 0.9985), 1)   
preds_test3 = np.multiply((preds_test3 > 1.5373), 1)   
preds_test4 = np.multiply((preds_test4 > 0.7750), 1) 

sumbit_prediction = (preds_test0+preds_test1+preds_test2+preds_test3+preds_test4)/5
threshold_best = 0.5
############### submission file
# only submit where all 1

t1 = time.time()
pred_dict = {idx: rle_encode(np.round(downsample(sumbit_prediction[i]) == 1)) for i, idx in enumerate(test_df.index.values)}
t2 = time.time()
