...
y_valid1 = np.array(train_df[train_df.index.isin(v_fold.values)].masks.map(upsample).map(np.round).tolist()).reshape(-1, img_size_target, img_size_target, 1)
...
model_name = "urestnet_deeps_16_lz_4_cy_"+str(i)+".model"
...
m = model.evaluate(x_valid2, [y_valid2_no_empty, y_valid2, y_valid2])
print(pd.Series(index=model.metrics_names, data=m))
"""
########### prediction starts from here 

preds_valid06 = predict_result(model,x_valid,img_size_target)
sumbit_prediction1 = predict_result(model,x_test,img_size_target)

preds_valid07 = predict_result(model,x_valid,img_size_target)
sumbit_prediction2 = predict_result(model,x_test,img_size_target)

preds_valid08 = predict_result(model,x_valid,img_size_target)
sumbit_prediction3 = predict_result(model,x_test,img_size_target)

preds_valid09 = predict_result(model,x_valid,img_size_target)
sumbit_prediction4 = predict_result(model,x_test,img_size_target)

preds_valid11 = predict_result(model,x_valid,img_size_target)
sumbit_prediction5 = predict_result(model,x_test,img_size_target)

preds_valid12 = predict_result(model,x_valid,img_size_target)
sumbit_prediction6 = predict_result(model,x_test,img_size_target)

preds_valid13 = predict_result(model,x_valid,img_size_target)
sumbit_prediction7 = predict_result(model,x_test,img_size_target)

preds_valid14 = predict_result(model,x_valid,img_size_target)
sumbit_prediction8 = predict_result(model,x_test,img_size_target)

preds_valid0 = (preds_valid06+preds_valid07+preds_valid08+preds_valid09)/4
preds_valid1 = (preds_valid11+preds_valid12+preds_valid13+preds_valid14)/4
                
final_prediction = np.concatenate([preds_valid0, preds_valid1])
final_truth = np.concatenate([y_valid0, y_valid1])
...
############### submission file
sumbit_prediction = (sumbit_prediction1+sumbit_prediction2+sumbit_prediction4+sumbit_prediction5+sumbit_prediction6
                     +sumbit_prediction7+sumbit_prediction8)/8
 
#np.save('submit_fold_0_1_21.npy', sumbit_prediction)
                     
submission_file = 'submission_all.csv'
threshold_best =0
