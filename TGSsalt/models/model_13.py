...
def my_main(batch_size, epochs, start_neurons_arg, dropout_arg, train_mask_threshold, lr_arg):
    ...
    fold = 3
    load_model_file = "/home/neha/Desktop/code/ML/Kaggle/TGSsalt/models/unet_best_fold_revised_sgd_woflip_lh_aug_"+str(fold)+".model"
    save_model_file = "/home/neha/Desktop/code/ML/Kaggle/TGSsalt/models/unet_best_fold_revised_sgd_woflip_lh_aug_batch16_"+str(fold)+".model"
    ...
    # remove dropout layers from the model
    model.get_layer('dropout_1').rate = 0.0  
    model.get_layer('dropout_2').rate = 0.0  
    model.get_layer('dropout_3').rate = 0.0  
    model.get_layer('dropout_4').rate = 0.0 
    model.get_layer('dropout_5').rate = 0.0 
    model.get_layer('spatial_dropout2d_1').rate = 0.0
    ...
    #lrate = Cosine_Scheduler(min_lr=1e-6, max_lr=1e-4,steps_per_epoch=st_per_epoch,lr_decay=0.9,cycle_length=5,mult_factor=1.5)
    #lrate = Cosine_Scheduler(min_lr=1e-6, max_lr=1e-4,steps_per_epoch=st_per_epoch,lr_decay=0.9,cycle_length=5,mult_factor=1.5)
    lrate = Cosine_Scheduler(min_lr=0.001, max_lr=0.01,steps_per_epoch=st_per_epoch,lr_decay=0.9,cycle_length=15,mult_factor=1.1)
    ...
    #reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric_2', mode = 'max',factor=0.3, patience=5, min_lr=0.0000001, verbose=1)
    #lrate = LearningRateScheduler(cosine_lr_2)
    #lrate = Cosine_Scheduler(min_lr=1e-6, max_lr=1e-4,steps_per_epoch=st_per_epoch,lr_decay=0.9,cycle_length=5,mult_factor=1.5)
    #lrate = Cosine_Scheduler(min_lr=1e-6, max_lr=1e-4,steps_per_epoch=st_per_epoch,lr_decay=0.9,cycle_length=5,mult_factor=1.5)
    #lrate = Cosine_Scheduler(min_lr=0.003, max_lr=0.03,steps_per_epoch=st_per_epoch,lr_decay=0.9,cycle_length=50,mult_factor=1.1)
    reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric_2', mode = 'max',factor=0.5, patience=6, min_lr=1e-10, verbose=1)
    model.fit_generator(generator(x_train2, y_train2, batch_size),
    ...
    '''
    lr_finder = LRFinder(min_lr=1e-7, max_lr=1e-1, steps_per_epoch=st_per_epoch, epochs=5)
