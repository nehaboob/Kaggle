...
def my_main(batch_size, epochs, start_neurons_arg, dropout_arg, train_mask_threshold, lr_arg):
    ...
    def upsample(img):# not used
        if img_size_ori == img_size_target:
            return img
        if img_size_target == 224:
            img = resize(img, (img_size_ori*2, img_size_ori*2), mode='constant', preserve_range=True)
            img = pad(img, [(11, 11), [11, 11]], mode='constant', constant_values=0)
            return img
        else:
    ...
    def downsample(img):# not used
        if img_size_ori == img_size_target:
            return img
        if img_size_target == 224:
            img = crop(img, [(11, 11), [11, 11]])
            img = resize(img, (img_size_ori, img_size_ori), mode='constant', preserve_range=True)
            return img
    ...
    def build_model(input_layer, start_neurons, DropoutRatio = 0.5):
        resnet_base = ResNet34(input_shape=(img_size_target,img_size_target,3), weights='imagenet', classes=1000, include_top=False)
        ...
        # 7 -> 14
        deconv4 = Conv2D(32, 2, activation = 'relu', padding = 'same')(UpSampling2D()(conv5))
        uconv4 = concatenate([deconv4, conv4], axis=-1)
        #conv_up6 = Dropout(DropoutRatio)(conv_up6)
        
        uconv4 = Conv2D(32, (3,3), activation=None, padding='same')(uconv4)
        uconv4 = residual_block(uconv4, 32)
        uconv4 = residual_block(uconv4, 32, True)
        
        # 14 -> 28
        deconv3 = Conv2D(32, 2, activation = 'relu', padding = 'same')(UpSampling2D()(uconv4))
        uconv3 = concatenate([deconv3, conv3], axis=-1)
        #uconv3 = Dropout(DropoutRatio)(uconv3)
        
        uconv3 = Conv2D(32, (3,3), activation=None, padding='same')(uconv3)
        uconv3 = residual_block(uconv3, 32)
        uconv3 = residual_block(uconv3, 32, True)
        
        # 28 -> 56
        deconv2 = Conv2D(32, 2, activation = 'relu', padding = 'same')(UpSampling2D()(uconv3))
        uconv2 = concatenate([deconv2, conv2], axis=-1)
        #uconv2 = Dropout(DropoutRatio)(uconv2)
        
        uconv2 = Conv2D(32, (3,3), activation=None, padding='same')(uconv2)
        uconv2 = residual_block(uconv2, 32)
        uconv2 = residual_block(uconv2, 32, True)
        
        # 56 -> 112
        deconv1 = Conv2D(32, 2, activation = 'relu', padding = 'same')(UpSampling2D()(uconv2))
        uconv1 = concatenate([deconv1, conv1], axis=-1)
        #uconv1 = Dropout(DropoutRatio)(uconv1)
        
        uconv1 = Conv2D(32, (3,3), activation=None, padding='same')(uconv1)
        uconv1 = residual_block(uconv1, 32)
        uconv1 = residual_block(uconv1, 32, True)
        
        # 112 -> 224
        deconv0 = Conv2D(32, 2, activation = 'relu', padding = 'same')(UpSampling2D()(uconv1))
        uconv0 = concatenate([deconv0, resnet_base.input], axis=-1)
        #uconv1 = Dropout(DropoutRatio)(uconv1)
        
        uconv0 = Conv2D(32, (3,3), activation=None, padding='same')(uconv0)
        uconv0 = residual_block(uconv0, 32)
        uconv0 = residual_block(uconv0, 32, True)
         
        # after last decoding block
        uconv0 = Conv2D(32, (3,3), activation='relu', padding='same')(uconv0)
        uconv0 = Conv2D(32, (2,2), activation='relu', padding='same')(uconv0)
        #output_layer_noActi = Conv2D(1, (1,1), activation=None, name='convd_1_1')(uconv0)
        
        image_pool = AveragePooling2D(pool_size=7)(conv5)
        ...
        hypercolumn = concatenate([
            uconv0,
            Lambda(lambda image: ktf.image.resize_images(image, (img_size_target, img_size_target)))(uconv1),
        ...
        deep_model = Model(inputs=resnet_base.input, outputs=[classification, hypercolumn, fusion])
    ...
    def cosine_lr_2(epoch):
        ...
        max_lr = 0.0002
        min_lr = 0.0001 
        time_e = np.linspace(0, 6.28, 60)
        cos_values = (np.cos(time_e)+1)/2
        cos_values = cos_values*(max_lr-min_lr)+min_lr
        
        return cos_values[epoch%40]
    ...
    def do_augmentation(X_train, y_train):
        ...
        iaa.OneOf([
                ...
                iaa.Noop(),
                iaa.Add((-0.08, 0.08), per_channel=0.5),
        ...
        seq = iaa.Sequential([
            ...
            iaa.OneOf([
                    iaa.Noop(),
                    iaa.Affine(scale=(0.9, 1.1), rotate=(-10, 10), translate_percent={"x": (-0.25, 0.25)}, mode='symmetric', cval=(0)),
                    iaa.Noop(),
                    iaa.PerspectiveTransform(scale=(0.04, 0.08)),
                    iaa.Noop()
                ]),
    ...
    fold = 3
    load_model_file = "/home/neha/Desktop/code/ML/Kaggle/TGSsalt/models/resent34_unet_bce_super_ls_"+str(fold)+".model"
    save_model_file = "/home/neha/Desktop/code/ML/Kaggle/TGSsalt/models/resent34_unet_bce_super_ds_"+str(fold)+".model"
    ...
    y_valid_no_empty = np.sum(y_valid, axis=(1,2,3))
    ...
    model.compile(optimizer='adam',loss=['binary_crossentropy', bce_dice_loss_non_empty, weighted_bce_dice_loss], loss_weights=[0.05, 0.1, 1.0])
    model.summary()
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric', mode = 'max',factor=0.5, patience=5, min_lr=1e-10, verbose=1)
    model.fit_generator(generator(x_train, y_train, batch_size),
                        validation_data=[x_valid, [y_valid_no_empty, y_valid, y_valid]], 
                        epochs=epochs,
    ...
    sgd= SGD(lr=lr_arg)
    model.compile(loss=lovasz_loss, optimizer=sgd, metrics=['accuracy', my_iou_metric, my_iou_metric_2])    
    model.summary()
