...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 10 09:55:59 2018
...
def my_metrics(_run, logs):
    ...
    _run.log_scalar("my_iou_metric_2", float(logs.get('my_iou_metric_2')))
    _run.log_scalar("val_my_iou_metric_2", float(logs.get('val_my_iou_metric_2')))
...
def my_main(batch_size, epochs, start_neurons_arg, dropout_arg, train_mask_threshold, lr_arg):
    ...
    def get_iou_vector(y_true_in, y_pred_in):   
    ...
    def iou_metric_batch_1(y_true_in, y_pred_in):
        batch_size = y_true_in.shape[0]
        metric = []
        for batch in range(batch_size):
    ...
    def iou_metric_batch_2(y_true_in, y_pred_in):
        ...
        ious = np.array([iou_metric_batch_1(y_true_in, y_pred_in > threshold) for threshold in thresholds])
    ...
    save_model_name = 'renet_best_10.model'
    ...
    """
    save_model_name = 'renet_best_10_hl.model'

    model = load_model(save_model_name, custom_objects={'my_iou_metric':my_iou_metric, 'weighted_bce_dice_loss': weighted_bce_dice_loss, 'my_iou_metric_2': my_iou_metric, 'lovasz_loss':lovasz_loss})
    ...
    model.compile(loss=lovasz_loss, optimizer=c, metrics=[my_iou_metric_2, my_iou_metric,'accuracy'])
    ...
    model_checkpoint = ModelCheckpoint(save_model_name,monitor='val_my_iou_metric_2', 
                                       mode = 'max', save_best_only=True, verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='val_my_iou_metric_2', mode = 'max',factor=0.5, patience=6, min_lr=0.0000001, verbose=1)
    #lrate = LearningRateScheduler(cosine_lr_2)
