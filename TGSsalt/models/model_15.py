...
def my_metrics(_run, logs):
    ...
    _run.log_scalar("my_iou_metric", float(logs.get('conv2d_47_my_iou_metric')))
    _run.log_scalar("val_my_iou_metric", float(logs.get('val_conv2d_47_my_iou_metric')))
    _run.result = float(logs.get('val_conv2d_47_my_iou_metric'))
...
def my_main(batch_size, epochs, start_neurons_arg, dropout_arg, train_mask_threshold, lr_arg):
    ...
    fold = 3
    load_model_file = "/home/neha/Desktop/code/ML/Kaggle/TGSsalt/models/urestnet_deeps_16_lz_cy_"+str(fold)+".model"
    save_model_file = "/home/neha/Desktop/code/ML/Kaggle/TGSsalt/models/urestnet_deeps_16_lz_2_cy_"+str(fold)+".model"
    ...
    sgd= SGD(lr=lr_arg)
    #adam= Adam(lr=lr_arg)
    model.compile(optimizer='adam',loss=['binary_crossentropy', bce_dice_loss_non_empty, weighted_bce_dice_loss], loss_weights=[0.05, 0.1, 1.0], metrics=['accuracy', my_iou_metric])
    model.summary()
    
    # model
    st_per_epoch = x_train.shape[0]//batch_size
    model_checkpoint = ModelCheckpoint(save_model_file,monitor='val_fusion_my_iou_metric', mode = 'max', save_best_only=True, verbose=1)
    ...
    """
    # model
    model = load_model(load_model_file,custom_objects={'my_iou_metric': my_iou_metric, 'weighted_bce_dice_loss':weighted_bce_dice_loss, 
                                                          'lovasz_loss':lovasz_loss, 
                                                          'bce_dice_loss_non_empty':bce_dice_loss_non_empty, 
                                                          'my_iou_metric_2':my_iou_metric_2, 
                                                          'ktf': ktf, 'img_size_target':img_size_target})
    #model.summary()
    ...
    model_checkpoint = ModelCheckpoint(save_model_file,monitor='val_conv2d_47_my_iou_metric_2', mode = 'max', save_best_only=True, verbose=1)
    reduce_lr = ReduceLROnPlateau(monitor='val_conv2d_47_my_iou_metric_2', mode = 'max',factor=0.5, patience=8, min_lr=1e-10, verbose=1)
    ...
    model = load_model(load_model_file,custom_objects={'my_iou_metric': my_iou_metric, 'weighted_bce_dice_loss':weighted_bce_dice_loss, 
                                                          'lovasz_loss':lovasz_loss, 
                                                          'lovasz_loss_no_empty': lovasz_loss_no_empty,
                                                          'bce_dice_loss_non_empty':bce_dice_loss_non_empty, 
                                                          'my_iou_metric_2':my_iou_metric_2, 
                                                          'ktf': ktf, 'img_size_target':img_size_target})
    ...
    #early_stopping = EarlyStopping(monitor='val_my_iou_metric_2', mode = 'max', patience=25, verbose=1)
    model_checkpoint = ModelCheckpoint(save_model_file,monitor='val_conv2d_47_my_iou_metric_2', mode = 'max', save_best_only=True, save_weights_only=True, verbose=1)
    #reduce_lr = ReduceLROnPlateau(monitor='val_conv2d_47_my_iou_metric_2', mode = 'max',factor=0.5, patience=8, min_lr=1e-10, verbose=1)
    lrate = Cosine_Scheduler(min_lr=0.001, max_lr=0.01, steps_per_epoch=st_per_epoch,lr_decay=0.9, cycle_length=30, mult_factor=1)
