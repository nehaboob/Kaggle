...
This exact verion gives LB: .31941
 ...
 predict first level prices only using houseing data
 predict the values and then add macro features and do egboost again
"""
...
percent_null = macro.isnull().mean(axis=0) > 0.20
...
train_merged = pd.merge(train, macro, how='left', on='timestamp')
test_merged = pd.merge(test, macro, how='left', on='timestamp')
...
# generate the prediction on test set, merge macro and retrin
val_predict = rfr.predict(dval)
test_predict = final.predict(dtest)

internal_chars = list(macro.columns.values)
june_index = train_merged.timestamp.dt.date >= datetime.date(2014, 6, 30)
pre_june_index = train_merged.timestamp.dt.date < datetime.date(2014, 6, 30)

second_stage_train = train_merged.loc[june_index, internal_chars]
second_stage_train = second_stage_train.drop(["timestamp"], axis=1)

second_stage_test = test_merged.loc[:, internal_chars]
second_stage_test = second_stage_test.drop(["timestamp"], axis=1)


second_target = train_merged.loc[june_index, 'price_doc'] 

second_stage_train['price_doc_1'] = val_predict
second_stage_test['price_doc_1'] = test_predict
...
dtrain = xgb.DMatrix(second_stage_train, second_target)
dtest = xgb.DMatrix(second_stage_test)
num_boost_rounds = 385  # This was the CV output, as earlier version shows
sfr = xgb.train(dict(xgb_params, silent=0), dtrain, num_boost_round= num_boost_rounds)
pred = sfr.predict(dtrain)
#print("Train error: {:.4f}, Test error: {:.4f}".format(rmsle_exp(pred, second_target)))


predictions = sfr.predict(dtest)
