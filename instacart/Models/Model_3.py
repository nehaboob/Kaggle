...
us_re = pd.concat([priors_orders_detail[priors_orders_detail['order_number'] > priors_orders_detail['_user_total_orders_made']/2].groupby('user_id').order_id.count().rename('_user_recent_product'),
...
d_test = xgboost.DMatrix(X_test.drop(['eval_set', 'user_id', 'order_id', 'reordered', 'product_id', 'aisle_id', 'department_id'], axis=1))
